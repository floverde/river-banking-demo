 ...
 *
 * @see https://github.com/jangalinski/kotlin-dataclass-mapper/blob/master/src/main/kotlin/DataClassMapper.kt
 * @author jangalinski (jan.galinski@holisticon.de)
...
class DataClassMapper<I : Any, O : Any>(private val inType: KClass<I>, private val outType: KClass<O>) : Mapper<I, O> {
    ...
    private val aliasNameProviders = mutableMapOf<String, String>()
    ...
    private fun argFor(parameter: KParameter, data: I): Any? {
        val sourceFieldName = this.getAlias(parameter.name)
        // get value from input data or apply a default value to the target class
        val value = inPropertiesByName[sourceFieldName]?.get(data)
        return if (value != null) {
            // if a special mapper is registered, use it, otherwise keep value
            this.fieldMappers[sourceFieldName]?.invoke(value) ?: value
        } else {
            this.targetParameterProviders[sourceFieldName]?.invoke()
        }
    }

    private fun getAlias(targetFieldName: String?): String? {
        return if (targetFieldName != null) {
            this.aliasNameProviders.getOrDefault(targetFieldName, targetFieldName)
        } else null;
    }
    ...
    fun registerAlias(sourceProperty: KProperty1<I, Any?>, targetProperty: KProperty1<O, Any?>): DataClassMapper<I, O> = apply {
        this.aliasNameProviders[targetProperty.name] = sourceProperty.name
    }

    fun registerAlias(sourcePropertyName: String, targetPropertyName: String): DataClassMapper<I, O> = apply {
        this.aliasNameProviders[targetPropertyName] = sourcePropertyName
    }

    fun registerConstant(targetProperty: KProperty1<O, Any?>, constant: Any): DataClassMapper<I, O> = apply {
        this.targetParameterSupplier(targetProperty) { constant }
    }

    fun registerConstant(targetProperty: String, constant: Any): DataClassMapper<I, O> = apply {
        this.targetParameterSupplier(targetProperty) { constant }
    }
